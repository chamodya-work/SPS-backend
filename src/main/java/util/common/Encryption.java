...
public class Encryption {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Encryption.class);  // ADDED: Logger for debugging encryption issues
    ...
    public boolean validateLogin(String userName, String password, String storedPassword) throws Exception {
        ...
        if (m_User.trim().isEmpty() || m_Pass.trim().isEmpty()) {  // ADDED: Check for empty results from checkPass (e.g., invalid chars), return false as in original
        ...
        try {
            ...
            return roundUp.equals(1.00000000d);  // CHANGED: Now format the ratio *after* division, matching original logic exactly
        } catch (NumberFormatException e) {  // ADDED: Catch parse exceptions (e.g., very large numbers exceeding double precision)
...
//public class Encryption {
//
//    public boolean validateLogin(String userName, String password, String storedEncryptedPassword) {
...
//            System.out.println("Validation result: " + (Math.abs(calculatedVal - storedVal) < 0.00000001));
