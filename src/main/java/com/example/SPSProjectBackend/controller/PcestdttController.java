...
public class PcestdttController {
    ...
    public ResponseEntity<Pcestdtt> update(
        ...
        if (updatedData.getResType() != null) existing.setResType(updatedData.getResType());
        if (updatedData.getUom() != null) existing.setUom(updatedData.getUom());
        if (updatedData.getUnitPrice() != null) existing.setUnitPrice(updatedData.getUnitPrice());
        if (updatedData.getEstimateQty() != null) existing.setEstimateQty(updatedData.getEstimateQty());
        if (updatedData.getEstimateCost() != null) existing.setEstimateCost(updatedData.getEstimateCost());
        if (updatedData.getReturnedQty() != null) existing.setReturnedQty(updatedData.getReturnedQty());
        if (updatedData.getReturnedCost() != null) existing.setReturnedCost(updatedData.getReturnedCost());
        if (updatedData.getApprovedQty() != null) existing.setApprovedQty(updatedData.getApprovedQty());
        if (updatedData.getDamageQty() != null) existing.setDamageQty(updatedData.getDamageQty());
